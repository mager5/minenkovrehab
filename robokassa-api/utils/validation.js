...
function validatePaymentParams(params) {
  ...
  if (
    params.phone &&
    !/^\+?[1-9]\d{1,14}$/.test(params.phone.replace(/[\s\-\(\)]/g, ''))
  ) {
  ...
  */
  ...
  if (!params.amount) {
    ...
    errors.push('Минимальная сумма платежа: 1 ₽');
  } else if (params.amount > 1000000) {
    errors.push('Максимальная сумма платежа: 1 000 000 ₽');
  }
...
function validateResultParams(params) {
  ...
  if (!params.SignatureValue) {
    ...
    typeof params.SignatureValue !== 'string' ||
    params.SignatureValue.length !== 32
  ) {
...
function validateEnvironment() {
  ...
  if (!process.env.ROBOKASSA_PASSWORD1) {
    warnings.push(
      'Переменная окружения ROBOKASSA_PASSWORD1 не установлена - генерация платежных ссылок может не работать'
    );
  }

  if (!process.env.ROBOKASSA_PASSWORD2) {
    warnings.push(
      'Переменная окружения ROBOKASSA_PASSWORD2 не установлена - проверка подписей Result URL может не работать'
    );
  }
  ...
  if (process.env.ROBOKASSA_TEST_MODE === 'true') {
    if (!process.env.ROBOKASSA_TEST_PASSWORD1) {
      warnings.push(
        'Тестовая переменная ROBOKASSA_TEST_PASSWORD1 не установлена - генерация тестовых платежных ссылок может не работать'
      );
    }
    if (!process.env.ROBOKASSA_TEST_PASSWORD2) {
      warnings.push(
        'Тестовая переменная ROBOKASSA_TEST_PASSWORD2 не установлена - проверка тестовых подписей Result URL может не работать'
      );
    }
  }

...
const TEST_CONFIG = {
  ...
  testCases: [
    ...
    },
    {
      ...
      amount: 100,
      description: 'Минимальный платеж 100 руб'
    },
    {
      name: 'Тест с большой суммой',
      amount: 50000,
      description: 'Крупный платеж 50000 руб'
    },
    {
...
async function testPaymentLink(testCase) {
  try {
    ...
    const response = await axios.post(`${TEST_CONFIG.baseUrl}/api/robokassa/create-payment`, {
    ...
    if (response.data.success && response.data.paymentUrl) {
      ...
      if (analysis.culture === 'ru') {
        console.log(`   ⚠️  ВНИМАНИЕ: Culture=ru может вызвать редирект на .kz`);
      ...
      console.log(`   ❌ Ошибка генерации ссылки: ${response.data.error || 'Неизвестная ошибка'}`);
      return { success: false, issue: 'generation_error', error: response.data.error };
    }
  } catch (error) {
    ...
    return { success: false, issue: 'request_error', error: error.message, details: error.response?.data };
  }
}

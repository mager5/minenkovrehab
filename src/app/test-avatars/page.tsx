  ...
  return (
    <div className="p-8">
      ...
      {avatars.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {avatars.map((avatar, index) => (
            <div key={avatar.avatar_id || index} className="border rounded-lg p-4 shadow">
              ...
              <div className="text-xs text-gray-500">
                <p><strong>Публичный:</strong> {avatar.is_public ? 'Да' : 'Нет'}</p>
                <p><strong>Статус:</strong> {avatar.status}</p>
                {avatar.default_voice && (
                  <p><strong>Голос по умолчанию:</strong> {avatar.default_voice}</p>
                )}
              </div>
            </div>
          ))}
      ...
      <div className="mt-8 p-4 bg-gray-100 rounded">
        ...
        <p><strong>Всего аватаров:</strong> {avatars.length}</p>
        <details className="mt-2">
          <summary className="cursor-pointer text-blue-600">Показать JSON</summary>

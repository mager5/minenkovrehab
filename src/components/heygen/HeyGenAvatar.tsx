...
export default function HeyGenAvatar({
  ...
  useEffect(() => {
    ...
    const handleIframeError = () => {
      setError(
        'Не удалось загрузить AI консультанта. Проверьте, что Vite сервер запущен на порту 3001.'
      );
  ...
  if (error) {
    return (
      <div
        className={`w-full h-96 flex items-center justify-center bg-gray-100 rounded-lg ${className || ''}`}
      >

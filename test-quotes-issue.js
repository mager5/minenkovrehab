...
const testCases = [
  ...
  },
  {
    ...
    description:
      "'Восстановительная программа коленного сустава после операции'",
  },
];
    ...
    try {
      ...
      if (response.statusCode === 200 && response.data.success) {
        ...
        if (encodedChars && encodedChars.length > 0) {
          console.log(
            `🔍 Найдены закодированные символы: ${encodedChars.join(', ')}`
          );
        }
      } else {
...
async function runTests() {
  ...
  try {
    await testEnvironment(
      'Локальная среда',
      'http://localhost:3001/api/robokassa/generate-payment-url',
      false
    );
  } catch (error) {
    console.log(

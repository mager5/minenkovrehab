...
class E2ETester {
  ...
  async testPages() {
    ...
    for (const pageInfo of pages) {
      try {
        await this.page.goto(this.baseUrl + pageInfo.url, { waitUntil: 'networkidle' });
        ...
        this.errors.push(`${pageInfo.name} - ошибка загрузки: ${error.message}`);
      }
    }
  }
  ...
  async checkPageElements(pageName) {
    try {
      ...
      this.warnings.push(`${pageName} - ошибка проверки элементов: ${error.message}`);
    }
  }
  ...
  async testContactForm() {
    try {
      ...
      if (submitButton) {
        ...
        if (isDisabled) {
          this.warnings.push('Кнопка отправки заблокирована после заполнения формы');
        } else {
      ...
      this.errors.push(`Ошибка тестирования контактной формы: ${error.message}`);
    }
  }
  ...
  async testBookingModal() {
    try {
      ...
      const modalTrigger = await this.page.$('button:has-text("Записаться"), button:has-text("Консультация"), [data-modal="booking"]');
      ...
      if (modalTrigger) {
        ...
        if (modal) {
          ...
          const closeButton = await this.page.$('button:has-text("×"), [aria-label="Close"], .close');
          if (closeButton) {
            await closeButton.click();
            await this.page.waitForTimeout(500);
            
            const modalAfterClose = await this.page.$('[role="dialog"], .modal');
            if (!modalAfterClose) {
      ...
      this.warnings.push(`Ошибка тестирования модального окна: ${error.message}`);
    }
  }
  ...
  async testCookieBanner() {
    try {
      ...
      const cookieBanner = await this.page.$('[data-testid="cookie-banner"], .cookie-banner');
      if (cookieBanner) {
        log.success('Cookie баннер - отображается');
        
        const acceptButton = await this.page.$('button:has-text("Принять"), button:has-text("Accept")');
        if (acceptButton) {
          await acceptButton.click();
          await this.page.waitForTimeout(500);
          
          const bannerAfterAccept = await this.page.$('[data-testid="cookie-banner"], .cookie-banner');
      ...
      this.warnings.push(`Ошибка тестирования cookie баннера: ${error.message}`);
    }
  }
    ...
    for (const viewport of viewports) {
      try {
        ...
        if (header && main && footer) {
          log.success(`${viewport.name} (${viewport.width}x${viewport.height}) - основные элементы отображаются`);
        } else {
          this.warnings.push(`${viewport.name} - некоторые элементы не отображаются`);
        }
        
      } catch (error) {
        this.warnings.push(`Ошибка тестирования ${viewport.name}: ${error.message}`);
      }
    }
  }
  ...
  async testAccessibility() {
    ...
    try {
      ...
      if (h1Elements.length === 1) {
        ...
        this.warnings.push(`На странице ${h1Elements.length} H1 заголовков (должен быть один)`);
      }
